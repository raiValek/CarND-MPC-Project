...
int latency_steps = (int)ceil(latency / (dt * 1000));
...
//
MPC::MPC() {
  predictedX.resize(N-1);
  predictedY.resize(N-1);
}
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // During latency the accelartion is clamped to the last value
  ...
  for (i = a_start+latency_steps; i < n_vars; i++) {
  ...
  for (size_t i = 1; i < N; ++i) {
    predictedX[i-1] = solution.x[x_start + i];
    predictedY[i-1] = solution.x[y_start + i];
  }

  last_delta = solution.x[delta_start+latency_steps];
  last_a = solution.x[a_start+latency_steps];

  // Return the next state values and the next valid actuator values.
  // Since the actuators won't respond during latency time, the next actuator
  // values are those after latency time.
  return {solution.x[x_start + 1],   solution.x[y_start + 1],
          solution.x[psi_start + 1], solution.x[v_start + 1],
          solution.x[cte_start + 1], solution.x[epsi_start + 1],
          solution.x[delta_start + latency_steps],   solution.x[a_start + latency_steps]};
}
